...
export class BookingController {
  ...
  static async createBooking(params, currentUser) {
    ...
    if (
      ...
      if (bookingData.stripeAccountId) {
        bookingData['onwerStripeId'] = tripOwner.stripeAccountId;
        bookingData['memberStripeId'] =
          bookingData.stripePaymentMethod.customer;
            ...
            try {
              ...
              if (booking.currentDue > 1) {
                ...
                if (booking.cryptoPaymentMethod) {
                  paymentIntent = booking.cryptoPaymentMethod;
                  booking.paymentRetryCount = 0;
                } else if (!booking.onwerStripeId) {
              ...
              if (paymentIntent) {
                if (!booking.cryptoPaymentMethod) {
                  booking.paymentHistory.push({
                    ...
                    currency: booking.currency,
                    //this gets filled in later with crypto payment details
                    paymentIntent: paymentIntent,
                  });
                }

                //we don't want to mark values as paid or zero them out for crypto payments since
                //they happen asynchronously later.
                bookingUpdate = {
                  paymentHistory: booking.paymentHistory,
                  paidAmout: booking.cryptoPaymentMethod
                    ? 0
                    : booking.currentDue,
                  currentDue: booking.cryptoPaymentMethod
                    ? booking.currentDue
                    : booking.pendingAmount,
                  pendingAmount: booking.cryptoPaymentMethod
                    ? booking.pendingAmount
                    : 0,
                  status: booking.cryptoPaymentMethod
                    ? 'paymentPending'
                    : 'approved',
                };
                ...
                if (!booking.cryptoPaymentMethod) {
                  //only send emails once payment is complete
